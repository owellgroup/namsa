  ...
  const actions: Action<ArtistWork>[] = [
    {
      ...
      onClick: (music) => {
        if (!music.fileUrl) {
          toast({ title: 'Audio Not Available', description: 'No audio file available for this track', variant: 'destructive' });
          return;
        }
  ...
  return (
    <DashboardLayout title="Music Library">
      <div className="space-y-6">
        ...
        />
        <MusicPlayerDialog open={playerOpen} onOpenChange={setPlayerOpen} track={playerTrack} />
      </div>
    </DashboardLayout>
  );
};
